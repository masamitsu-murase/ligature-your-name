...
module LigatureYourName
  ...
  class Deflater
    ...
    def call(env)
      ...
      encoding = Rack::Utils.select_best_encoding(%w(gzip identity),
      ...
      when "gzip"
        ...
        if body.respond_to?(:to_path) && File.file?(body.to_path)
          mtime = File.mtime(body.to_path).to_i unless mtime
          compressed_body = StringIO.new("")
          gzip = ::Zlib::GzipWriter.new(compressed_body)
          gzip.mtime = mtime if mtime
          File.open(body.to_path, "rb") do |input|
            IO.copy_stream(input, gzip)
          end
          gzip.close
          return [status, headers, [compressed_body.string]]
        end
